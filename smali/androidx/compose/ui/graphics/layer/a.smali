.class public final Landroidx/compose/ui/graphics/layer/a;
.super Ljava/lang/Object;
.source "SourceFile"


# instance fields
.field public final a:Lpf2;

.field public b:Lud1;

.field public c:Landroidx/compose/ui/unit/LayoutDirection;

.field public d:Lfa2;

.field public final e:Lfa2;

.field public f:Landroid/graphics/Outline;

.field public g:Z

.field public h:J

.field public i:J

.field public j:F

.field public k:Lth1;

.field public l:Lcc;

.field public m:Lcc;

.field public n:Z

.field public o:Lwg0;

.field public p:Lvb;

.field public q:I

.field public final r:Lxl0;

.field public s:Z

.field public t:J

.field public u:J

.field public v:J

.field public w:Z

.field public x:Landroid/graphics/RectF;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    sget-object v0, Landroid/os/Build;->FINGERPRINT:Ljava/lang/String;

    sget-object v1, Ljava/util/Locale;->ROOT:Ljava/util/Locale;

    invoke-virtual {v0, v1}, Ljava/lang/String;->toLowerCase(Ljava/util/Locale;)Ljava/lang/String;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    const-string v1, "robolectric"

    invoke-virtual {v0, v1}, Ljava/lang/Object;->equals(Ljava/lang/Object;)Z

    return-void
.end method

.method public constructor <init>(Lpf2;)V
    .locals 5

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    sget-object v0, Ltl1;->a:Lwd1;

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->b:Lud1;

    sget-object v0, Landroidx/compose/ui/unit/LayoutDirection;->Ltr:Landroidx/compose/ui/unit/LayoutDirection;

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->c:Landroidx/compose/ui/unit/LayoutDirection;

    sget-object v0, Landroidx/compose/ui/graphics/layer/GraphicsLayer$drawBlock$1;->INSTANCE:Landroidx/compose/ui/graphics/layer/GraphicsLayer$drawBlock$1;

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->d:Lfa2;

    new-instance v0, Landroidx/compose/ui/graphics/layer/GraphicsLayer$clipDrawBlock$1;

    invoke-direct {v0, p0}, Landroidx/compose/ui/graphics/layer/GraphicsLayer$clipDrawBlock$1;-><init>(Landroidx/compose/ui/graphics/layer/a;)V

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->e:Lfa2;

    const/4 v0, 0x1

    iput-boolean v0, p0, Landroidx/compose/ui/graphics/layer/a;->g:Z

    const-wide/16 v0, 0x0

    iput-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->h:J

    const-wide v2, 0x7fc000007fc00000L    # 2.247117487993712E307

    iput-wide v2, p0, Landroidx/compose/ui/graphics/layer/a;->i:J

    new-instance v4, Lxl0;

    invoke-direct {v4}, Ljava/lang/Object;-><init>()V

    iput-object v4, p0, Landroidx/compose/ui/graphics/layer/a;->r:Lxl0;

    const/4 v4, 0x0

    iput-boolean v4, p1, Lpf2;->s:Z

    invoke-virtual {p1}, Lpf2;->a()V

    iput-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->t:J

    iput-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->u:J

    iput-wide v2, p0, Landroidx/compose/ui/graphics/layer/a;->v:J

    return-void
.end method


# virtual methods
.method public final a()V
    .locals 17

    move-object/from16 v0, p0

    iget-object v1, v0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    iget-object v2, v1, Lpf2;->c:Landroid/graphics/RenderNode;

    iget-boolean v3, v0, Landroidx/compose/ui/graphics/layer/a;->g:Z

    const/4 v4, 0x0

    if-eqz v3, :cond_9

    iget-boolean v3, v0, Landroidx/compose/ui/graphics/layer/a;->w:Z

    if-nez v3, :cond_1

    iget v5, v1, Lpf2;->n:F

    const/4 v6, 0x0

    cmpl-float v5, v5, v6

    if-lez v5, :cond_0

    goto :goto_0

    :cond_0
    iput-boolean v4, v1, Lpf2;->s:Z

    invoke-virtual {v1}, Lpf2;->a()V

    const/4 v3, 0x0

    invoke-virtual {v2, v3}, Landroid/graphics/RenderNode;->setOutline(Landroid/graphics/Outline;)Z

    iput-boolean v4, v1, Lpf2;->g:Z

    invoke-virtual {v1}, Lpf2;->a()V

    goto/16 :goto_2

    :cond_1
    :goto_0
    iget-object v5, v0, Landroidx/compose/ui/graphics/layer/a;->l:Lcc;

    const/4 v6, 0x1

    if-eqz v5, :cond_6

    iget-object v3, v0, Landroidx/compose/ui/graphics/layer/a;->x:Landroid/graphics/RectF;

    if-nez v3, :cond_2

    new-instance v3, Landroid/graphics/RectF;

    invoke-direct {v3}, Landroid/graphics/RectF;-><init>()V

    iput-object v3, v0, Landroidx/compose/ui/graphics/layer/a;->x:Landroid/graphics/RectF;

    :cond_2
    instance-of v7, v5, Lcc;

    if-eqz v7, :cond_5

    iget-object v7, v5, Lcc;->a:Landroid/graphics/Path;

    invoke-virtual {v7, v3, v4}, Landroid/graphics/Path;->computeBounds(Landroid/graphics/RectF;Z)V

    iget-object v7, v0, Landroidx/compose/ui/graphics/layer/a;->f:Landroid/graphics/Outline;

    if-nez v7, :cond_3

    new-instance v7, Landroid/graphics/Outline;

    invoke-direct {v7}, Landroid/graphics/Outline;-><init>()V

    iput-object v7, v0, Landroidx/compose/ui/graphics/layer/a;->f:Landroid/graphics/Outline;

    :cond_3
    invoke-static {v7, v5}, Lk12;->e0(Landroid/graphics/Outline;Lcc;)V

    invoke-virtual {v7}, Landroid/graphics/Outline;->canClip()Z

    move-result v8

    xor-int/2addr v8, v6

    iput-boolean v8, v0, Landroidx/compose/ui/graphics/layer/a;->n:Z

    iput-object v5, v0, Landroidx/compose/ui/graphics/layer/a;->l:Lcc;

    iget v5, v1, Lpf2;->h:F

    invoke-virtual {v7, v5}, Landroid/graphics/Outline;->setAlpha(F)V

    invoke-virtual {v3}, Landroid/graphics/RectF;->width()F

    move-result v5

    invoke-static {v5}, Ljava/lang/Math;->round(F)I

    invoke-virtual {v3}, Landroid/graphics/RectF;->height()F

    move-result v3

    invoke-static {v3}, Ljava/lang/Math;->round(F)I

    invoke-virtual {v2, v7}, Landroid/graphics/RenderNode;->setOutline(Landroid/graphics/Outline;)Z

    iput-boolean v6, v1, Lpf2;->g:Z

    invoke-virtual {v1}, Lpf2;->a()V

    iget-boolean v3, v0, Landroidx/compose/ui/graphics/layer/a;->n:Z

    if-eqz v3, :cond_4

    iget-boolean v3, v0, Landroidx/compose/ui/graphics/layer/a;->w:Z

    if-eqz v3, :cond_4

    iput-boolean v4, v1, Lpf2;->s:Z

    invoke-virtual {v1}, Lpf2;->a()V

    invoke-virtual {v2}, Landroid/graphics/RenderNode;->discardDisplayList()V

    goto/16 :goto_2

    :cond_4
    iget-boolean v2, v0, Landroidx/compose/ui/graphics/layer/a;->w:Z

    iput-boolean v2, v1, Lpf2;->s:Z

    invoke-virtual {v1}, Lpf2;->a()V

    goto/16 :goto_2

    :cond_5
    const-string v0, "Unable to obtain android.graphics.Path"

    invoke-static {v0}, Lel;->r(Ljava/lang/String;)V

    return-void

    :cond_6
    iput-boolean v3, v1, Lpf2;->s:Z

    invoke-virtual {v1}, Lpf2;->a()V

    iget-object v3, v0, Landroidx/compose/ui/graphics/layer/a;->f:Landroid/graphics/Outline;

    if-nez v3, :cond_7

    new-instance v3, Landroid/graphics/Outline;

    invoke-direct {v3}, Landroid/graphics/Outline;-><init>()V

    iput-object v3, v0, Landroidx/compose/ui/graphics/layer/a;->f:Landroid/graphics/Outline;

    :cond_7
    move-object v7, v3

    iget-wide v8, v0, Landroidx/compose/ui/graphics/layer/a;->u:J

    invoke-static {v8, v9}, Lz91;->F(J)J

    move-result-wide v8

    iget-wide v10, v0, Landroidx/compose/ui/graphics/layer/a;->h:J

    iget-wide v12, v0, Landroidx/compose/ui/graphics/layer/a;->i:J

    const-wide v14, 0x7fc000007fc00000L    # 2.247117487993712E307

    cmp-long v3, v12, v14

    if-nez v3, :cond_8

    goto :goto_1

    :cond_8
    move-wide v8, v12

    :goto_1
    const/16 v3, 0x20

    shr-long v12, v10, v3

    long-to-int v5, v12

    invoke-static {v5}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v12

    invoke-static {v12}, Ljava/lang/Math;->round(F)I

    move-result v12

    const-wide v13, 0xffffffffL

    and-long/2addr v10, v13

    long-to-int v10, v10

    invoke-static {v10}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v11

    invoke-static {v11}, Ljava/lang/Math;->round(F)I

    move-result v11

    invoke-static {v5}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v5

    move-wide v15, v13

    shr-long v13, v8, v3

    long-to-int v3, v13

    invoke-static {v3}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v13

    add-float/2addr v13, v5

    invoke-static {v13}, Ljava/lang/Math;->round(F)I

    move-result v5

    invoke-static {v10}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v10

    and-long/2addr v8, v15

    long-to-int v13, v8

    invoke-static {v13}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v8

    add-float/2addr v8, v10

    invoke-static {v8}, Ljava/lang/Math;->round(F)I

    move-result v8

    move v9, v11

    move v11, v8

    move v8, v12

    iget v12, v0, Landroidx/compose/ui/graphics/layer/a;->j:F

    move v10, v5

    invoke-virtual/range {v7 .. v12}, Landroid/graphics/Outline;->setRoundRect(IIIIF)V

    iget v5, v1, Lpf2;->h:F

    invoke-virtual {v7, v5}, Landroid/graphics/Outline;->setAlpha(F)V

    invoke-static {v3}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v3

    invoke-static {v3}, Ljava/lang/Math;->round(F)I

    invoke-static {v13}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v3

    invoke-static {v3}, Ljava/lang/Math;->round(F)I

    invoke-virtual {v2, v7}, Landroid/graphics/RenderNode;->setOutline(Landroid/graphics/Outline;)Z

    iput-boolean v6, v1, Lpf2;->g:Z

    invoke-virtual {v1}, Lpf2;->a()V

    :cond_9
    :goto_2
    iput-boolean v4, v0, Landroidx/compose/ui/graphics/layer/a;->g:Z

    return-void
.end method

.method public final b()V
    .locals 15

    iget-boolean v0, p0, Landroidx/compose/ui/graphics/layer/a;->s:Z

    if-eqz v0, :cond_6

    iget v0, p0, Landroidx/compose/ui/graphics/layer/a;->q:I

    if-nez v0, :cond_6

    iget-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->r:Lxl0;

    iget-object v1, v0, Lxl0;->b:Ljava/lang/Object;

    check-cast v1, Landroidx/compose/ui/graphics/layer/a;

    if-eqz v1, :cond_0

    iget v2, v1, Landroidx/compose/ui/graphics/layer/a;->q:I

    add-int/lit8 v2, v2, -0x1

    iput v2, v1, Landroidx/compose/ui/graphics/layer/a;->q:I

    invoke-virtual {v1}, Landroidx/compose/ui/graphics/layer/a;->b()V

    const/4 v1, 0x0

    iput-object v1, v0, Lxl0;->b:Ljava/lang/Object;

    :cond_0
    iget-object v0, v0, Lxl0;->d:Ljava/lang/Object;

    check-cast v0, Lpe4;

    if-eqz v0, :cond_5

    iget-object v1, v0, Landroidx/collection/g;->b:[Ljava/lang/Object;

    iget-object v2, v0, Landroidx/collection/g;->a:[J

    array-length v3, v2

    add-int/lit8 v3, v3, -0x2

    if-ltz v3, :cond_4

    const/4 v4, 0x0

    move v5, v4

    :goto_0
    aget-wide v6, v2, v5

    not-long v8, v6

    const/4 v10, 0x7

    shl-long/2addr v8, v10

    and-long/2addr v8, v6

    const-wide v10, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long/2addr v8, v10

    cmp-long v8, v8, v10

    if-eqz v8, :cond_3

    sub-int v8, v5, v3

    not-int v8, v8

    ushr-int/lit8 v8, v8, 0x1f

    const/16 v9, 0x8

    rsub-int/lit8 v8, v8, 0x8

    move v10, v4

    :goto_1
    if-ge v10, v8, :cond_2

    const-wide/16 v11, 0xff

    and-long/2addr v11, v6

    const-wide/16 v13, 0x80

    cmp-long v11, v11, v13

    if-gez v11, :cond_1

    shl-int/lit8 v11, v5, 0x3

    add-int/2addr v11, v10

    aget-object v11, v1, v11

    check-cast v11, Landroidx/compose/ui/graphics/layer/a;

    iget v12, v11, Landroidx/compose/ui/graphics/layer/a;->q:I

    add-int/lit8 v12, v12, -0x1

    iput v12, v11, Landroidx/compose/ui/graphics/layer/a;->q:I

    invoke-virtual {v11}, Landroidx/compose/ui/graphics/layer/a;->b()V

    :cond_1
    shr-long/2addr v6, v9

    add-int/lit8 v10, v10, 0x1

    goto :goto_1

    :cond_2
    if-ne v8, v9, :cond_4

    :cond_3
    if-eq v5, v3, :cond_4

    add-int/lit8 v5, v5, 0x1

    goto :goto_0

    :cond_4
    invoke-virtual {v0}, Lpe4;->e()V

    :cond_5
    iget-object p0, p0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    iget-object p0, p0, Lpf2;->c:Landroid/graphics/RenderNode;

    invoke-virtual {p0}, Landroid/graphics/RenderNode;->discardDisplayList()V

    :cond_6
    return-void
.end method

.method public final c(Lwl1;)V
    .locals 13

    iget-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->r:Lxl0;

    iget-object v1, v0, Lxl0;->b:Ljava/lang/Object;

    check-cast v1, Landroidx/compose/ui/graphics/layer/a;

    iput-object v1, v0, Lxl0;->c:Ljava/lang/Object;

    iget-object v1, v0, Lxl0;->d:Ljava/lang/Object;

    check-cast v1, Lpe4;

    if-eqz v1, :cond_1

    invoke-virtual {v1}, Landroidx/collection/g;->c()Z

    move-result v2

    if-eqz v2, :cond_1

    iget-object v2, v0, Lxl0;->e:Ljava/lang/Object;

    check-cast v2, Lpe4;

    if-nez v2, :cond_0

    sget-object v2, Lkp5;->a:Lpe4;

    new-instance v2, Lpe4;

    invoke-direct {v2}, Lpe4;-><init>()V

    iput-object v2, v0, Lxl0;->e:Ljava/lang/Object;

    :cond_0
    invoke-virtual {v2, v1}, Lpe4;->j(Landroidx/collection/g;)V

    invoke-virtual {v1}, Lpe4;->e()V

    :cond_1
    const/4 v1, 0x1

    iput-boolean v1, v0, Lxl0;->a:Z

    iget-object p0, p0, Landroidx/compose/ui/graphics/layer/a;->d:Lfa2;

    invoke-interface {p0, p1}, Lfa2;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    const/4 p0, 0x0

    iput-boolean p0, v0, Lxl0;->a:Z

    iget-object p1, v0, Lxl0;->c:Ljava/lang/Object;

    check-cast p1, Landroidx/compose/ui/graphics/layer/a;

    if-eqz p1, :cond_2

    iget v1, p1, Landroidx/compose/ui/graphics/layer/a;->q:I

    add-int/lit8 v1, v1, -0x1

    iput v1, p1, Landroidx/compose/ui/graphics/layer/a;->q:I

    invoke-virtual {p1}, Landroidx/compose/ui/graphics/layer/a;->b()V

    :cond_2
    iget-object p1, v0, Lxl0;->e:Ljava/lang/Object;

    check-cast p1, Lpe4;

    if-eqz p1, :cond_7

    invoke-virtual {p1}, Landroidx/collection/g;->c()Z

    move-result v0

    if-eqz v0, :cond_7

    iget-object v0, p1, Landroidx/collection/g;->b:[Ljava/lang/Object;

    iget-object v1, p1, Landroidx/collection/g;->a:[J

    array-length v2, v1

    add-int/lit8 v2, v2, -0x2

    if-ltz v2, :cond_6

    move v3, p0

    :goto_0
    aget-wide v4, v1, v3

    not-long v6, v4

    const/4 v8, 0x7

    shl-long/2addr v6, v8

    and-long/2addr v6, v4

    const-wide v8, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long/2addr v6, v8

    cmp-long v6, v6, v8

    if-eqz v6, :cond_5

    sub-int v6, v3, v2

    not-int v6, v6

    ushr-int/lit8 v6, v6, 0x1f

    const/16 v7, 0x8

    rsub-int/lit8 v6, v6, 0x8

    move v8, p0

    :goto_1
    if-ge v8, v6, :cond_4

    const-wide/16 v9, 0xff

    and-long/2addr v9, v4

    const-wide/16 v11, 0x80

    cmp-long v9, v9, v11

    if-gez v9, :cond_3

    shl-int/lit8 v9, v3, 0x3

    add-int/2addr v9, v8

    aget-object v9, v0, v9

    check-cast v9, Landroidx/compose/ui/graphics/layer/a;

    iget v10, v9, Landroidx/compose/ui/graphics/layer/a;->q:I

    add-int/lit8 v10, v10, -0x1

    iput v10, v9, Landroidx/compose/ui/graphics/layer/a;->q:I

    invoke-virtual {v9}, Landroidx/compose/ui/graphics/layer/a;->b()V

    :cond_3
    shr-long/2addr v4, v7

    add-int/lit8 v8, v8, 0x1

    goto :goto_1

    :cond_4
    if-ne v6, v7, :cond_6

    :cond_5
    if-eq v3, v2, :cond_6

    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    :cond_6
    invoke-virtual {p1}, Lpe4;->e()V

    :cond_7
    return-void
.end method

.method public final d()Lth1;
    .locals 14

    iget-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->k:Lth1;

    iget-object v1, p0, Landroidx/compose/ui/graphics/layer/a;->l:Lcc;

    if-eqz v0, :cond_0

    return-object v0

    :cond_0
    if-eqz v1, :cond_1

    new-instance v0, Lzq4;

    invoke-direct {v0, v1}, Lzq4;-><init>(Lcc;)V

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->k:Lth1;

    return-object v0

    :cond_1
    iget-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->u:J

    invoke-static {v0, v1}, Lz91;->F(J)J

    move-result-wide v0

    iget-wide v2, p0, Landroidx/compose/ui/graphics/layer/a;->h:J

    iget-wide v4, p0, Landroidx/compose/ui/graphics/layer/a;->i:J

    const-wide v6, 0x7fc000007fc00000L    # 2.247117487993712E307

    cmp-long v6, v4, v6

    if-nez v6, :cond_2

    goto :goto_0

    :cond_2
    move-wide v0, v4

    :goto_0
    const/16 v4, 0x20

    shr-long v5, v2, v4

    long-to-int v5, v5

    invoke-static {v5}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v6

    const-wide v7, 0xffffffffL

    and-long/2addr v2, v7

    long-to-int v2, v2

    invoke-static {v2}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v2

    shr-long v9, v0, v4

    long-to-int v3, v9

    invoke-static {v3}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v3

    add-float/2addr v3, v6

    and-long/2addr v0, v7

    long-to-int v0, v0

    invoke-static {v0}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v0

    add-float v9, v0, v2

    iget v0, p0, Landroidx/compose/ui/graphics/layer/a;->j:F

    const/4 v1, 0x0

    cmpl-float v1, v0, v1

    if-lez v1, :cond_3

    new-instance v1, Lbr4;

    invoke-static {v0}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v5

    int-to-long v10, v5

    invoke-static {v0}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v0

    int-to-long v12, v0

    shl-long v4, v10, v4

    and-long/2addr v7, v12

    or-long v10, v4, v7

    move v7, v2

    move v8, v3

    invoke-static/range {v6 .. v11}, Lr05;->d(FFFFJ)Lyl5;

    move-result-object v0

    invoke-direct {v1, v0}, Lbr4;-><init>(Lyl5;)V

    goto :goto_1

    :cond_3
    move v7, v2

    move v8, v3

    new-instance v1, Lar4;

    new-instance v0, Ljb5;

    invoke-direct {v0, v6, v7, v8, v9}, Ljb5;-><init>(FFFF)V

    invoke-direct {v1, v0}, Lar4;-><init>(Ljb5;)V

    :goto_1
    iput-object v1, p0, Landroidx/compose/ui/graphics/layer/a;->k:Lth1;

    return-object v1
.end method

.method public final e()V
    .locals 10

    iget-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->b:Lud1;

    iget-object v1, p0, Landroidx/compose/ui/graphics/layer/a;->c:Landroidx/compose/ui/unit/LayoutDirection;

    iget-object v2, p0, Landroidx/compose/ui/graphics/layer/a;->e:Lfa2;

    iget-object v3, p0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    iget-object v4, v3, Lpf2;->b:Lwg0;

    iget-object v5, v3, Lpf2;->c:Landroid/graphics/RenderNode;

    invoke-virtual {v5}, Landroid/graphics/RenderNode;->beginRecording()Landroid/graphics/RecordingCanvas;

    move-result-object v6

    :try_start_0
    iget-object v7, v3, Lpf2;->a:Lul5;

    iget-object v8, v7, Lul5;->f:Ljava/lang/Object;

    check-cast v8, Lp9;

    iget-object v9, v8, Lp9;->a:Landroid/graphics/Canvas;

    iput-object v6, v8, Lp9;->a:Landroid/graphics/Canvas;

    iget-object v6, v4, Lwg0;->f:Lfk;

    invoke-virtual {v6, v0}, Lfk;->G(Lud1;)V

    invoke-virtual {v6, v1}, Lfk;->H(Landroidx/compose/ui/unit/LayoutDirection;)V

    iput-object p0, v6, Lfk;->i:Ljava/lang/Object;

    iget-wide v0, v3, Lpf2;->d:J

    invoke-virtual {v6, v0, v1}, Lfk;->I(J)V

    invoke-virtual {v6, v8}, Lfk;->F(Lug0;)V

    invoke-interface {v2, v4}, Lfa2;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    iget-object p0, v7, Lul5;->f:Ljava/lang/Object;

    check-cast p0, Lp9;

    iput-object v9, p0, Lp9;->a:Landroid/graphics/Canvas;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    invoke-virtual {v5}, Landroid/graphics/RenderNode;->endRecording()V

    return-void

    :catchall_0
    move-exception p0

    invoke-virtual {v5}, Landroid/graphics/RenderNode;->endRecording()V

    throw p0
.end method

.method public final f(J)V
    .locals 4

    iget-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->v:J

    invoke-static {v0, v1, p1, p2}, Llm4;->e(JJ)Z

    move-result v0

    if-nez v0, :cond_1

    iput-wide p1, p0, Landroidx/compose/ui/graphics/layer/a;->v:J

    const-wide v0, 0x7fffffff7fffffffL

    and-long/2addr v0, p1

    const-wide v2, 0x7fc000007fc00000L    # 2.247117487993712E307

    cmp-long v0, v0, v2

    iget-object p0, p0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    iget-object p0, p0, Lpf2;->c:Landroid/graphics/RenderNode;

    if-nez v0, :cond_0

    invoke-virtual {p0}, Landroid/graphics/RenderNode;->resetPivot()Z

    return-void

    :cond_0
    const/16 v0, 0x20

    shr-long v0, p1, v0

    long-to-int v0, v0

    invoke-static {v0}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v0

    invoke-virtual {p0, v0}, Landroid/graphics/RenderNode;->setPivotX(F)Z

    const-wide v0, 0xffffffffL

    and-long/2addr p1, v0

    long-to-int p1, p1

    invoke-static {p1}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result p1

    invoke-virtual {p0, p1}, Landroid/graphics/RenderNode;->setPivotY(F)Z

    :cond_1
    return-void
.end method

.method public final g(JJ)V
    .locals 6

    const/16 v0, 0x20

    shr-long v1, p1, v0

    long-to-int v1, v1

    const-wide v2, 0xffffffffL

    and-long/2addr p1, v2

    long-to-int p1, p1

    iget-object p0, p0, Landroidx/compose/ui/graphics/layer/a;->a:Lpf2;

    iget-object p2, p0, Lpf2;->c:Landroid/graphics/RenderNode;

    shr-long v4, p3, v0

    long-to-int v0, v4

    add-int/2addr v0, v1

    and-long/2addr v2, p3

    long-to-int v2, v2

    add-int/2addr v2, p1

    invoke-virtual {p2, v1, p1, v0, v2}, Landroid/graphics/RenderNode;->setPosition(IIII)Z

    invoke-static {p3, p4}, Lz91;->F(J)J

    move-result-wide p1

    iput-wide p1, p0, Lpf2;->d:J

    return-void
.end method

.method public final h(JFJ)V
    .locals 2

    iget-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->h:J

    invoke-static {v0, v1, p1, p2}, Llm4;->e(JJ)Z

    move-result v0

    if-eqz v0, :cond_1

    iget-wide v0, p0, Landroidx/compose/ui/graphics/layer/a;->i:J

    invoke-static {v0, v1, p4, p5}, Lq36;->c(JJ)Z

    move-result v0

    if-eqz v0, :cond_1

    iget v0, p0, Landroidx/compose/ui/graphics/layer/a;->j:F

    cmpg-float v0, v0, p3

    if-nez v0, :cond_1

    iget-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->l:Lcc;

    if-eqz v0, :cond_0

    goto :goto_0

    :cond_0
    return-void

    :cond_1
    :goto_0
    const/4 v0, 0x0

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->k:Lth1;

    iput-object v0, p0, Landroidx/compose/ui/graphics/layer/a;->l:Lcc;

    const/4 v0, 0x1

    iput-boolean v0, p0, Landroidx/compose/ui/graphics/layer/a;->g:Z

    const/4 v0, 0x0

    iput-boolean v0, p0, Landroidx/compose/ui/graphics/layer/a;->n:Z

    iput-wide p1, p0, Landroidx/compose/ui/graphics/layer/a;->h:J

    iput-wide p4, p0, Landroidx/compose/ui/graphics/layer/a;->i:J

    iput p3, p0, Landroidx/compose/ui/graphics/layer/a;->j:F

    invoke-virtual {p0}, Landroidx/compose/ui/graphics/layer/a;->a()V

    return-void
.end method
